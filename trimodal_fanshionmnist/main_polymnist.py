...
import models
import objective
from utils import Logger, Timer, save_model_light, save_vars, unpack_data_polyfmnist, get_10_polyFMNIST_samples
import os
import shutil
import torch
...
parser.add_argument('--experiment', type=str, default='mmvae_polyfmnist', metavar='E',
                    help='experiment name')
parser.add_argument('--model', type=str, default='polyfmnist_3modalities', metavar='M',
...
parser.add_argument('--batch-size', type=int, default=64, metavar='N',
                    help='batch size for data (default: 64)')
...
parser.add_argument('--beta', type=float, default=1.0, metavar='S',
                    help='random seed (default: 1)')
parser.add_argument('--tmpdir', type=str, default='../pfm_data') #../polym_data_dumy
parser.add_argument('--outputdir', type=str, default='./outputs')
parser.add_argument('--inception_module_path', type=str, default='../pfm_data/pfm/pt_inception-2015-12-05-6726825d.pth')
parser.add_argument('--pretrained_clfs_dir', type=str, default='../pfm_data/pfm/trained_clfs_polyMNIST')
...
datadir = os.path.join(args.tmpdir, "pfm")
...
def train(epoch, agg):
    ...
    for i, dataT in enumerate(train_loader):
        data, labels_batch = unpack_data_polyfmnist(dataT, device=device)
        optimizer.zero_grad()
        loss = -objective(epoch, model, data, K=args.K, test=False)
        loss.backward()
        optimizer.step()
        b_loss += loss.item()
...
def test(epoch, agg):
    ...
    b_loss = 0
    with torch.no_grad():
        test_selected_samples = get_10_polyFMNIST_samples(test_loader.dataset,
                                                     num_testing_images=test_loader.dataset.__len__(), device=device)
        for i, dataT in enumerate(test_loader):
            data, _ = unpack_data_polyfmnist(dataT, device=device)
            loss = -objective(epoch, model, data, K=args.K, test=True)
            b_loss += loss.item()
            if i == 0:
...
def cross_coherence():
    ...
    with torch.no_grad():
        for i, dataT in enumerate(test_loader):
            data, targets = unpack_data_polyfmnist(dataT, device)  # needs to be sent to device
...
def unconditional_coherence_and_lr(clf_lr):
    ...
    correct = 0
    total = 0
    lr_acc_m0_u, lr_acc_m1_u, lr_acc_m2_u = [], [], []
    lr_acc_m0_w, lr_acc_m1_w, lr_acc_m2_w = [], [], []
    lr_acc_m0_z, lr_acc_m1_z, lr_acc_m2_z = [], [], []
    accuracies_lr = {'u':{},
                     'z':{},
                     'w':{}}
    with torch.no_grad():
        for i, dataT in enumerate(test_loader):
            ...
            data, targets = unpack_data_polyfmnist(dataT, device)
...
def classify_latents_nl(mod, epoch):
    ...
    for i, dataT in enumerate(train_loader):
        data, labels = unpack_data_polyfmnist(dataT, device=device)
        data_batch = data[mod]
        with torch.no_grad():
...
def calculate_fid_routine(datadir, fid_path, num_fid_samples, epoch):
    ...
    with torch.no_grad():
        ...
        fid_randm_list = []
        fid_condgen_list = []
        for modality_target in ['m{}'.format(m) for m in range(3)]:
            file_activations_real = os.path.join(args.tmpdir, 'pfm', 'test',
            ...
            for modality_source in ['m{}'.format(m) for m in range(3)]:
...
def train_clf_lr(dl):
    ...
    for i, dataT_lr in enumerate(dl):
        data, labels_batch = unpack_data_polyfmnist(dataT_lr, device=device)
        b_size = data[0].size(0)
    ...
    with Timer('MM-VAE') as t:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if epoch % 25 == 0:
                ...
                calculate_fid_routine(datadir, fid_path, 10000, epoch) #10000
        writer.flush()
        writer.close()
