...
class MMVAE(nn.Module):
    def __init__(self, device, prior_dist, params, *vaes):
            ...
            for d, vae in enumerate(self.vaes):
                if e != d:  # fill-in off-diagonal
                    if self.params.variant == 'mmvaeplus':
                        ...
                        if epoch < 6:
                            latents_w = vae.sample_w_con_fid(us.size()[0], vae.out_w)
                        else:
                            latents_w = vae.sample_w_con(us.size()[0], vae.out_w)
                        if not self.params.no_cuda and torch.cuda.is_available():
                            latents_w.to(self.device)
    ...
    def generate(self, N):
        ...
        with torch.no_grad():
            ...
            X = self.means_z(self.idle_outz_input)[indx]
            qz_x_mean, qz_x_lv = self.generate_pzx_params(X)
            latents_z = self.pz(qz_x_mean, qz_x_lv).rsample().view(N, 1, -1)
            for d, vae in enumerate(self.vaes):
                latents_w = vae.sample_w_uncon(N).view(N,1,-1)
    ...
    def reconstruct_and_cross_reconstruct_forw(self, data):
        ...
        for e, us in enumerate(uss):
            ...
            for d, vae in enumerate(self.vaes):
                if e != d:  # fill-in off-diagonal
                    latents_w_new = vae.sample_w_con(us.size()[0], vae.out_w)
                    us = torch.cat((latents_w_new, latents_z), dim=-1)
    ...
    def reconstruct_and_cross_reconstruct_fid_forw(self, data):
        qu_xs, uss = [], []
        # initialise cross-modal matrix
        px_us = [[None for _ in range(len(self.vaes))] for _ in range(len(self.vaes))]
        # pw = self.pz(torch.zeros(1, self.params.latent_dim_w), torch.ones(1, self.params.latent_dim_w))
        ...
        for e, us in enumerate(uss):
            ...
            for d, vae in enumerate(self.vaes):
                if e != d:  # fill-in off-diagonal
                    latents_w_new = vae.sample_w_con_fid(us.size()[0], vae.out_w)
                    us = torch.cat((latents_w_new, latents_z), dim=-1)
                    px_us[e][d] = vae.px_u(*vae.dec(us))
        return qu_xs, px_us, uss

    def reconstruct_and_cross_reconstruct_fid(self, data):
        self.eval()
        with torch.no_grad():
            _, px_us, _ = self.reconstruct_and_cross_reconstruct_fid_forw(data)
            # ------------------------------------------------
            # cross-modal matrix of reconstructions
            recons = [[get_mean(px_u) for px_u in r] for r in px_us]
        return recons
