...
from datasets_PolyFM import PolyFMDataset
...
class PolyFMNIST_3modalities(MMVAE):
    def __init__(self, params, device):
        super(PolyFMNIST_3modalities, self).__init__(device, dist.Laplace, params, PolyMNIST(params,device, 0.0035, 0.001, False), 
                                                    PolyMNIST(params,device, 0.0011,0.001, False), PolyMNIST(params,device, -0.0018, 0.001, False))
        ...
        self.modelName = 'polyfmnist-3modalities'
    ...
    def getDataLoaders(self, batch_size, shuffle=True, device='cuda'):
        ...
        unim_train_datapaths = [self.params.tmpdir + "/pfm/train/" + "m" + str(i) for i in [0, 1, 2]]
        unim_test_datapaths = [self.params.tmpdir + "/pfm/test/" + "m" + str(i) for i in [0, 1, 2]]
        dataset_PolyMNIST_train = PolyFMDataset(unim_train_datapaths, transform=tx)
        dataset_PolyMNIST_test = PolyFMDataset(unim_test_datapaths, transform=tx)
    ...
    def generate(self):
        N = 100
        outputs = []
        samples_list = super(PolyFMNIST_3modalities, self).generate(N)
    ...
    def reconstruct_for_fid(self, data, savedir, i):
        recons_mat = super(PolyFMNIST_3modalities, self).reconstruct_and_cross_reconstruct_fid([d for d in data])
    ...
    def cross_generate(self, data):
        ...
        for i in range(10):
            recons_mat = super(PolyFMNIST_3modalities, self).reconstruct_and_cross_reconstruct_fid([d[:N] for d in data])
            for r, recons_list in enumerate(recons_mat):
                for o, recon in enumerate(recons_list):
                      recon = recon.squeeze(0).cpu()
