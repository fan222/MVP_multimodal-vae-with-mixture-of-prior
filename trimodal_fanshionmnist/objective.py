...
def _m_iwae(epoch, model, x, K=1, test=False):
    ...
    if test:
        ...
        qu_xs, px_us, uss = model(epoch, x, K)
    qz_xs, qw_xs = [], []
    ...
    for r, qu_x in enumerate(qu_xs):
        ...
        lpw = model.vaes[r].log_p_w(ws)
        lpz = model.log_prob_z(zs)
